...
# Периодический вывод, чтобы бот не "уснул" на Replit
def keep_alive():
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Привет! Я буду присылать тебе утреннюю викторину!")
...
if __name__ == '__main__':
    ...
    if not TOKEN:
        raise ValueError(
            "❌ Токен не найден. Убедитесь, что он указан в файле .env")

    print(f"✅ Токен: {TOKEN[:5]}...{TOKEN[-5:]}"
          )  # Только подтверждение, что токен считан
    ...
    scheduler.add_job(send_quiz, 'cron', hour=8, minute=0,
                      args=[application])  # Каждое утро в 8:00
    scheduler.start()
